...
if(isMultiply){
    ...
    answer = Math.floor(Math.random() * 13) - 6;

    a = answer * b;
}

        const key = `${a}${isMultiply ? "×" : "÷"}${b}`;
...
localStorage.setItem("resultCoin", coin);
    ...
    localStorage.setItem("stage4Score",score);
...
console.log("保存後coin =", localStorage.getItem("coin"));
    ...
    const best = Number(localStorage.getItem("stage4Best")) || 0;

if(score > best){

    localStorage.setItem("stage4Best",score);

}

    if(score >= 8){

        localStorage.setItem("stage4Clear","true");

    }

    localStorage.setItem("resultStage", "stage4");
